...
public class SimulationRunner : MonoBehaviour
{
    ...
    [Header("Prefabs animali (devono avere Animal + AnimalPreset)")]
    public GameObject preyPrefab;
    public GameObject predatorPrefab;
 
    [Header("Container per animali runtime")]
    ...
    private void TryAttack(Animal predator, AnimalState pState)
    {
        ...
        if (pState.attackCooldown > 0f) return;   // rispetta il cooldown
        ...
        foreach (var prey in _queryBuf)
        {
            ...
            // Knockback: la preda viene spinta nella direzione opposta al predatore
    ...
    private void TryReproduce(Animal parentA, Animal parentB)
    {
        ...
        var state  = ReproductionSystem.CreateOffspring(parentA.State, genes, _settings, _nextId++);
    ...
    private void SpawnAnimalFromEntry(SpawnEntry entry)
    {
        ...
        if (animal == null)
        {
            Debug.LogError($"[SimulationRunner] Prefab {prefab.name} is missing Animal component!");
            Destroy(go);
            return;
        }
        ...
        // Animali iniziali partono con cooldown pieno (stessa logica della prole)
